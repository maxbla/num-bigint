...
fn bitand_pos_neg(a: &mut SmallVec<SmallVecArray>, b: &[BigDigit]) {
    let mut carry_b = 1;
...
fn bitand_neg_pos(a: &mut SmallVec<SmallVecArray>, b: &[BigDigit]) {
    let mut carry_a = 1;
    for (ai, &bi) in a.iter_mut().zip(b.iter()) {
...
fn bitand_neg_neg(a: &mut SmallVec<SmallVecArray>, b: &[BigDigit]) {
    let mut carry_a = 1;
    let mut carry_b = 1;
    let mut carry_and = 1;
    for (ai, &bi) in a.iter_mut().zip(b.iter()) {
...
fn bitor_pos_neg(a: &mut SmallVec<SmallVecArray>, b: &[BigDigit]) {
    let mut carry_b = 1;
    let mut carry_or = 1;
    for (ai, &bi) in a.iter_mut().zip(b.iter()) {
        let twos_b = negate_carry(bi, &mut carry_b);
...
fn bitor_neg_pos(a: &mut SmallVec<SmallVecArray>, b: &[BigDigit]) {
    let mut carry_a = 1;
    let mut carry_or = 1;
    for (ai, &bi) in a.iter_mut().zip(b.iter()) {
        let twos_a = negate_carry(*ai, &mut carry_a);
...
fn bitor_neg_neg(a: &mut SmallVec<SmallVecArray>, b: &[BigDigit]) {
    let mut carry_a = 1;
    let mut carry_b = 1;
    let mut carry_or = 1;
    for (ai, &bi) in a.iter_mut().zip(b.iter()) {
...
// answer is neg, has length of longest with a possible carry
fn bitxor_pos_neg(a: &mut SmallVec<SmallVecArray>, b: &[BigDigit]) {
    let mut carry_b = 1;
    let mut carry_xor = 1;
    for (ai, &bi) in a.iter_mut().zip(b.iter()) {
        let twos_b = negate_carry(bi, &mut carry_b);
...
// answer is neg, has length of longest with a possible carry
fn bitxor_neg_pos(a: &mut SmallVec<SmallVecArray>, b: &[BigDigit]) {
    let mut carry_a = 1;
    let mut carry_xor = 1;
    for (ai, &bi) in a.iter_mut().zip(b.iter()) {
        let twos_a = negate_carry(*ai, &mut carry_a);
...
fn bitxor_neg_neg(a: &mut SmallVec<SmallVecArray>, b: &[BigDigit]) {
    let mut carry_a = 1;
    let mut carry_b = 1;
    for (ai, &bi) in a.iter_mut().zip(b.iter()) {
        let twos_a = negate_carry(*ai, &mut carry_a);
